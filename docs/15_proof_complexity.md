...
**Лемма 15.7.3 (PIT-аксиомы ⇒ EF p-симулирует IPS).** Если EF имеет полиномиальные доказательства PIT-аксиом для класса схем, содержащего IPS-сертификаты, то EF p-симулирует IPS (Grochow–Pitassi 2014).

*Доказательство (эскиз).* IPS-опровержение задано схемой $C(x,y)$ с тождествами
$C(x,0)\equiv 0$ и $C(x,f(x))\equiv 1$. PIT-аксиомы позволяют EF доказывать
корректность таких тождеств (равенство полиномов) и их следствия на булевом кубе.
Подставляя любое булево присваивание $x$, EF выводит, что если все клаузы
истинны, то $f(x)=0$ и тогда $C(x,0)=0$, но одновременно $C(x,f(x))=1$,
что противоречит; значит $\neg\varphi$ — тавтология. $\square$
