...
Q39: Tseitin(Grid) depth‑gap. Есть $k=O(\\log n)$ протокол для column‑summing (пересечение=1 XOR + dyadic‑merge, §16.144–§16.145) и «easy case» для соседних столбцов при фиксированных dyadic‑полосах (§16.146). Caveat: Håstad’86 Remark 6 — схемы; для формул нужен отдельный PARITY‑upper (§16.147). Узел: держать глубину $d\\approx\\log n/\\log\\log n$ без накопления $O(\\log n)$ (иначе барьер §16.132).
...
Последние: Сжатие/канонизация → Инвариант → Сжатие/канонизация → Эквивалентность → Модельный стресс‑тест
