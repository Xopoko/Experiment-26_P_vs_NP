...
Q39: depth≈$\\log n/\\log\\log n$ Frege needs XOR‑sim (§16.124–§16.126). Column‑summing границы = $O(1)$ row‑interval’ов (§16.134), но фиксированный базис невозможен (rank $n$; §16.135); dyadic‑префикс ⇒ $O(\\log n)$ merge, merge via §16.130 (§16.136–§16.137). Schoenfield: $\\neg$ синтаксичен ⇒ можно брать $\\neg P$ (slack OK; §16.138–§16.141). Representability PARITY pinned (§16.142). Toy: $S_1\\cap S_2=\\{x\\}$ ⇒ общий block‑базис $k\\le k_1+k_2+1$ ⇒ XOR‑шаг via §16.130 (§16.143). Узел: base‑change/row↔column без $k^2$ (барьер §16.132).
...
Последние: Двойственность/комплементы → Трейд‑офф → Трейд‑офф → Эквивалентность → Сжатие/канонизация
