...
Q39: Tseitin(Grid) depth‑gap. Grid‑Gauss редуцируется к XOR‑шагам в block‑форме с $k=O(\\log n)$ (пересечение=1 + dyadic‑merge, §16.144–§16.146). Узел: representability PARITY — для формул множитель $(d-1)$ в показателе **неустраним** (DNF‑композиция §16.148 + matching lower bound Rossman’16 §16.149), поэтому при $d\\approx\\log n/\\log\\log n$ и $M=\\mathrm{poly}(n)$ один PARITY‑блок покрывает лишь $n^{o(1)}$. Дальше: сверить, что HR называют «size $M$ formulas» в §1.2 (нет ли иной меры размера), иначе пересобрать редукцию Q39/узел глубины.
...
Последние: Сжатие/канонизация → Эквивалентность → Модельный стресс‑тест → Трейд‑офф → Ссылка/барьер
