...
Q39: Tseitin(Grid) depth‑gap. Стратегия Gauss на grid редуцируется к XOR‑шагам в block‑форме с $k=O(\\log n)$ (пересечение=1 + dyadic‑merge, §16.144–§16.146). Узел: representability PARITY на пороге — Håstad’86 Remark 6 это схемы (§16.147), а явная DNF‑композиция для формул даёт лишь $m\\le (\\Theta((\\log M)/(d-1)))^{d-1}$ (§16.148), т.е. при $M=\\mathrm{poly}(n)$ покрывает $n^{o(1)}$. Дальше: либо найти сильный формульный PARITY‑upper для растущего $d$, либо фиксировать барьер; затем возвращаться к контролю глубины.
...
Последние: Инвариант → Сжатие/канонизация → Эквивалентность → Модельный стресс‑тест → Трейд‑офф
