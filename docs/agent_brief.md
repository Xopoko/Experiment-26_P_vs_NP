...
Q39: Tseitin(Grid) depth‑gap. Block‑Gauss → XOR‑шаги в block‑форме с $k=O(\\log n)$ (§16.144–§16.146). Барьер representability: для tree‑формул Rossman’16 + мост HR‑depth→De Morgan (§16.150) дают блок $\\le(\\Theta((\\log M)/(d-1)))^{d-1}$; HR §1.2 “$(\\log M)^{d-1}$ при size $M$” — лишь константно‑глубинная эвристика без DAG (§16.151). Дальше: понять, где LB Håstad’20 опирается на fan‑out 1 и что меняется для circuit‑Frege.
...
Последние: Модельный стресс‑тест → Трейд‑офф → Ссылка/барьер → Эквивалентность → Уточнение формулировки
