...
Q39: Tseitin(Grid) depth‑gap. Grid‑Gauss редуцируется к XOR‑шагам в block‑форме с $k=O(\\log n)$ (пересечение=1 + dyadic‑merge, §16.144–§16.146). Узел: representability PARITY — для tree‑формул Rossman’16 (tight) + мост HR‑depth→De Morgan (§16.150) убивают блок $(\\log M)^{d-1}$ на пороге $d\\approx\\log n/\\log\\log n$ (при $M=\\mathrm{poly}(n)$ покрывается лишь $n^{o(1)}$). Дальше: проверить, не подразумевает ли HR §1.2 DAG/схемы под «formulas of size $M$»; если нет — пересобирать Q39 без «широких паритетов».
...
Последние: Канонизация → Модельный стресс‑тест → Трейд‑офф → Ссылка/барьер → Эквивалентность
