...
Q39: missing — depth‑$d\\approx\\log n/\\log\\log n$ Frege‑симуляция одного XOR‑add (§16.124–§16.126). Column‑summing: границы прямоугольников = $O(1)$ row‑interval сегментов (§16.134), но фиксированный «segment‑basis» невозможен даже в 1D (ранг $n$; §16.135). Dyadic‑разбивка префикса обновляется за $O(\\log n)$ merge‑операций (§16.136); сам merge — «easy case» при наличии блок‑паритетов (§16.137), узел — их поддержка/переиспользование по шагам.
...
Последние: Инвариант → Эквивалентность → Коммуникация/ранг → Сжатие/канонизация → Эквивалентность
