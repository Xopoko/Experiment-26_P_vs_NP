...
Q39: depth≈$\\log n/\\log\\log n$ Frege needs XOR‑sim (§16.124–§16.126). Column‑summing: границы = $O(1)$ row‑interval’ов (§16.134), локальный шаг $|\\delta\\cap E(v)|=1$ (§16.144); dyadic‑канонизация даёт протокол поддержания $k=O(\\log n)$ (XOR‑пересечение=1 + $O(\\log n)$ disjoint‑merge; §16.145). Schoenfield: $\\neg$ синтаксичен ⇒ можно брать $\\neg P$ (slack OK; §16.138–§16.141). Representability PARITY pinned (§16.142). Узел: сложение соседних столбцов/row↔column base‑change без $k^2$ (барьер §16.132).
...
Последние: Трейд‑офф → Эквивалентность → Сжатие/канонизация → Инвариант → Сжатие/канонизация
