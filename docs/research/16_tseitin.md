# P vs NP — исследовательские шаги 16.85–16.119 (Tseitin / proof complexity)
...
- `Линза:` Трейд‑офф.
- `Утверждение:` В обозначениях Galesi–Itsykson–Riazanov–Sofronova (2019) (GIRS’19) пусть $t:=\\mathrm{tw}(G)$.
  Тогда из доказательства Thm. 19 следует более явная форма:
  существует константа $a>0$, такая что для всех достаточно больших $d$ формула $\\mathrm{Tseitin}(G,f)$ имеет bounded‑depth Frege‑рефутацию
  глубины $O(d)$ и размера
  $$\\mathrm{poly}(|\\mathrm{Tseitin}(G,f)|)\\cdot 2^{t^{a/d}}.$$
  В частности, если $t\\ge 2$ и
  $$d\\ \\ge\\ \\frac{a\\,\\log t}{\\log\\log t},$$
  то существует polynomial‑size рефутация (так как тогда $t^{a/d}=\\log t$ и $2^{t^{a/d}}=t$).
  Для семейств bounded‑degree графов с $t=\\Theta(n)$ (grid/экспандеры) это даёт polynomial‑size уже при $d=\\Theta(\\log n/\\log\\log n)$,
  что согласуется по порядку с нижними оценками 16.92 и Thm. 18 (16.97).
- `Доказательство:` в доказательстве Thm. 19 (GIRS’19, p. 14) прямо написано, что существует константа $c$ и рефутация глубины $3d+O(1)$
  размера $\\mathrm{poly}(|\\mathrm{Tseitin}(G,f)|)\\cdot 2^{(10\\,\\Delta(G)^2\\,t)^{c/d}}$.
  Если $t>\\Delta(G)$, то там же переписывают это как $\\mathrm{poly}(|\\mathrm{Tseitin}(G,f)|)\\cdot 2^{(10t)^{3c/d}}$,
  а значит для некоторой константы $a$ имеем верхнюю оценку вида $\\mathrm{poly}(|\\mathrm{Tseitin}(G,f)|)\\cdot 2^{t^{a/d}}$.
  Далее, при $d=a\\,\\log t/\\log\\log t$ получаем
  $$t^{a/d}=t^{\\log\\log t/\\log t}=\\exp(\\log\\log t)=\\log t,$$
  и потому $2^{t^{a/d}}=2^{\\log t}=t$.
- `Toy‑тест:` при $d=\\Theta(\\log t)$ имеем $t^{a/d}=\\exp(O(1))$ и тем более получаем polynomial‑size; критичное место — масштаб $d\\approx(\\log t)/(\\log\\log t)$.
- `Статус:` доказано (закрывает Q35).
- `Барьер‑чек:` r — неприменимо, NP — неприменимо, alg — неприменимо.
- `Следующий шаг:` сверить параметр $d$ (в статье глубина $3d+O(1)$) с глубиной в 16.92/16.97 и переписать итог в терминах
  числа переменных/клауз $N$ для grid (выполнено в §16.116).
...
- `Утверждение:` Пусть $G$ — $n\\times n$ grid и $T:=\\mathrm{Tseitin}(G,f)$ — соответствующая Tseitin‑CNF.
  ...
  Тогда polynomial‑size bounded‑depth Frege‑рефутации $T$ существуют при глубине
  $$\\mathrm{depth}=O\\!\\left(\\frac{\\log N}{\\log\\log N}\\right),$$
  и, с другой стороны, любая polynomial‑size bounded‑depth Frege‑рефутация требует
  $$\\mathrm{depth}=\\Omega\\!\\left(\\frac{\\log N}{\\log\\log N}\\right).$$
  То есть для Tseitin(Grid) минимальная глубина polynomial‑size bounded‑depth Frege равна $\\Theta(\\log N/\\log\\log N)$ (по порядку).
- `Доказательство:`
  1) (**Upper bound.**) Для grid имеем $t:=\\mathrm{tw}(G)=\\Theta(n)$ и $\\Delta(G)=4$.
     По Thm. 19 (GIRS’19) в форме §16.115 существует константа $a>0$ такая, что для всех достаточно больших параметров $d$
     есть Frege‑рефутация глубины $3d+O(1)$ размера $\\mathrm{poly}(|T|)\\cdot 2^{t^{a/d}}$.
     Берём $d:=\\Theta(\\log t/\\log\\log t)$, тогда $2^{t^{a/d}}=\\mathrm{poly}(t)$ (см. расчёт в §16.115), и потому размер остаётся polynomial.
     Так как $t=\\Theta(n)$ и $N=\\Theta(n^2)$, получаем
     $$3d+O(1)=\\Theta\\!\\left(\\frac{\\log n}{\\log\\log n}\\right)=\\Theta\\!\\left(\\frac{\\log N}{\\log\\log N}\\right).$$
  2) (**Lower bound.**) По Håstad (2020), Cor. 6.6 (см. §16.92), для Tseitin(Grid$_{n,n}$) любая polynomial‑size bounded‑depth Frege‑рефутация
     требует глубины $\\Omega(\\log n/\\log\\log n)$. Подставляя $N=\\Theta(n^2)$, получаем также
     $\\Omega(\\log N/\\log\\log N)$ (так как $\\log n=\\tfrac12\\log N+O(1)$ и $\\log\\log n=\\log\\log N+O(1)$).
...
- `Статус:` доказано (закрывает Q36).
- `Барьер‑чек:` r — неприменимо, NP — неприменимо, alg — неприменимо.
- `Следующий шаг:` если нужно для «главного текста», вынести 2–3 строки: «для grid глубина polynomial‑size bounded‑depth Frege равна
  $\\Theta(\\log N/\\log\\log N)$» со ссылками на Håstad’20 и GIRS’19 (16.92+16.115–16.116).
...
- `Утверждение:` Для Tseitin(Grid$_{n,n}$) в bounded‑depth Frege:
  ...
  2) (**Верхняя константа только через параметр $c$.**) Из доказательства GIRS’19 Thm. 19 (см. §16.115) следует существование константы $c>0$,
     такой что polynomial‑size гарантируется при глубине
     $$\\mathrm{depth}\\ge \\frac{9c\\,\\log n}{\\log\\log n}+O(1).$$
  Следовательно, сравнение «в явных числах» упирается в то, что $c$ в GIRS’19 не фиксирован в тексте: известно лишь, что обе границы имеют порядок
  $\\Theta(\\log n/\\log\\log n)$, а нижняя константа $1/59$ явна.
- `Доказательство:`
  1) Пункт (1) — это ровно Cor. 6.6 из Håstad (2020), как записано в §16.92.
  2) Для grid имеем $t:=\\mathrm{tw}(G)=\\Theta(n)$ и $\\Delta(G)=4$.
     По доказательству Thm. 19 (GIRS’19, p. 14; см. §16.115) существует $c$ и рефутация глубины $3d+O(1)$ размера
     $$\\mathrm{poly}(|T|)\\cdot 2^{(10t)^{3c/d}},$$
     откуда polynomial‑size получается при выборе $d:=3c\\,\\log(10t)/\\log\\log t=\\Theta(\\log n/\\log\\log n)$
     (тогда $(10t)^{3c/d}=\\log(10t)$ и $2^{(10t)^{3c/d}}=10t$), и глубина равна $3d+O(1)=9c\\,\\log(10t)/\\log\\log t+O(1)$.
- `Toy‑тест:` на уровне «порядка роста» константа 3 в глубине $3d+O(1)$ лишь домножает требуемый $d$ на 3; существенный вопрос — значение $c$.
- `Статус:` доказано (сравнение сведено к явной константе 59 и неявной константе $c$).
- `Барьер‑чек:` r — неприменимо, NP — неприменимо, alg — неприменимо.
- `Следующий шаг:` либо найти в литературе верхнюю оценку с явной численной константой (без «существует $c$»),
  либо вручную распаковать $c$ из доказательства Thm. 19 (трекинг O‑констант в Claim 28/Thm. 26–27).
...
- `Toy‑тест:` для grid $n\\times n$ имеем $\\Delta=4$ и $\\mathrm{tw}=\\Theta(n)$, значит при выборе $d\\asymp 4\\log n/\\log\\log n$
  получаем экспоненту $(\\Theta(n))^{4/d}=\\Theta(\\log n)$ и потому polynomial‑size при глубине $\\asymp 12\\log n/\\log\\log n$ (для больших $n$).
- `Статус:` доказано (явная константа в экспоненте; закрывает «существует $c$» в Q39 до проверяемого числа).
- `Барьер‑чек:` r — неприменимо (верхняя оценка в ограниченной модели доказательств), NP — неприменимо, alg — неприменимо.
- `Следующий шаг:` проверить, можно ли убрать техническое условие $X^{1/d}\\ge\\max(2d,6)$ или улучшить константу $4$ (напр. другим выбором $t_i$),
  и затем сравнить получившийся явный upper‑коэффициент для grid с 59 из Håstad’20.
