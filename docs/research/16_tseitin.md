...
### 16.186. Исследовательский шаг (proof): Håstad’20 Thm. 6.5 ⇒ не существует “универсального” poly‑size XOR‑step в режиме $d=\\Theta(\\log n/\\log\\log n)$ для grid‑скелета
...
- `Утверждение (Q39.S5-lb-xor-step-hastad20):`
  ...
  Это противоречит Håstad’20 Thm. 6.5 (любой depth‑$d$ refutation требует размера $\\exp(\\Omega(n^{1/(58(d+1))}))$).
  Следовательно, в таком каркасе **хотя бы один** XOR‑шаг обязан иметь super‑polynomial размер при $d=\\Theta(\\log n/\\log\\log n)$.
- `Доказательство:`
  ...
  4) (Противоречие и “плохой шаг”.) Это противоречит пункту (1). Значит в любом таком XOR‑каркасе найдётся шаг размера хотя бы
  $$\\exp\\bigl(\\Omega(n^{1/(58(d+1))})\\bigr)/\\mathrm{poly}(n).$$
- `Toy‑тест:` при $d=(\\kappa+o(1))\\frac{\\log n}{\\log\\log n}$ имеем $n^{1/(58(d+1))}=\\exp(\\Theta(\\log\\log n))=\\log^{\\Theta(1)}n$,
  так что нижняя оценка на “плохой шаг” имеет вид $\\exp(\\log^{\\Theta(1)}n)/\\mathrm{poly}(n)$ (квазиполиномиальный масштаб).
- `Статус:` доказано (барьер: “универсальный poly XOR‑step” в этом depth‑режиме невозможен; хотя бы один шаг должен быть тяжёлым).
