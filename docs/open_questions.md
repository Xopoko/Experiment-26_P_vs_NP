...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: попробовать применить §16.130 к column‑summing на grid — найти фиксированное разбиение с $k=\\mathrm{polylog}(n)$, в котором все промежуточные уравнения являются union‑of‑blocks и сами блок‑паритеты выводимы без «скрытого» Gaussian elimination (или точно зафиксировать, где это ломается).
