...
- [ ] **Q39 (сделать верхнюю константу для Tseitin(Grid) явной, без «существует $c$»):**
  ...
  Прогресс: в §16.118 показано, что в GIRS’19 Lemma 24/25 можно взять явную константу $c=21$; значит оставшаяся “неявность”
  сидит только в переходе Claim 28/Theorem 27 → форме $2^{(10\\Delta^2\\mathrm{tw})^{c/d}}$ в доказательстве Thm. 19.
  Следующий шаг: переписать Claim 28/Theorem 27 без big‑O (подставив явные константы из Lemma 4, 22, 25) и получить численное $c$ в Thm. 19,
  после чего сравнить явный upper‑коэффициент с 59 из Håstad’20.
