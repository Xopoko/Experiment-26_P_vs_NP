...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  ...
  - `NextStepID:` Q39.S82-2k-two-strip-chain-strip-support-rowcol-2d-prefix-global-order-microsteps-alternating-bothstrips-local-block-monotone-2d-strict-prefix-frontier-unit-onestrip-alternating-global-order-fixed-schedule-two-phase-blocks-interleaved-anchored-shifted-balanced-rowcol-swap-fixedpair
  ...
  - `Update:` toy‑контрпример при $k=2$: при симметрии между полосами
    (row/column swap) остаются две разные ненулевые проекции ⇒ ранг 2.
    См. `formal/WIP/Work.lean` (Q39_unit_frontier_blocks_anchored_shifted_balanced_rowcolswap_vec1/vec2).
  - `Use:` следующий тест — fixed‑pair row/column swap (одна и та же пара строк/колонок на каждом шаге).
  ...
  - `PublicSurface:` `formal/WIP/Work.lean` (Q39_rank2_unit_frontier_blocks_anchored_shifted_balanced_rowcolswap)
...
- [ ] **Q43 (flat local‑EF(s): существуют ли “малые” evaluations для poly‑size доказательств?):**
  - `Priority:` P0
  - `Status:` ACTIVE
  - `LastStepID:` Q43.S217-flat-eval-hr-depth-range-constants-a0-c1c2-apply-thm41
  - `NextStepID:` Q43.S218-flat-eval-hr-depth-range-constants-a0-c1c2-log2-verify-regime
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` no
  - `PublicSurface:` `formal/WIP/Work.lean` (Q43_thm41_log2_threshold_c1_explicit/Q43_thm41_log2_denom_c2_explicit)
  - `Success:` либо схема построения cost‑$t$ evaluations (с $t=\mathrm{polylog}(n)$) для каждой строки flat local‑EF(s)‑доказательства, либо точная точка поломки (где multi‑switching/representation требует nesting или глобальной поддержки)
  - `Lens:` Эквивалентность (уровни ↔ глубина).
  - `Artifact:` Proof.
  - `Update:` определён log2‑порог Thm. 4.1 с явным `c1=27,680,440,320,000`
    и зафиксирован `c2=11,381,760,000` в соответствующем знаменателе.
    См. `formal/WIP/Work.lean`.
  - `Use:` следующий шаг — проверить, что этот log2‑порог выполняется в нужном режиме $d$ (с переводом в $|F|$).
