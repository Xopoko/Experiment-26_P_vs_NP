...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Representation‑часть теперь опирается на точную формулировку из того же §1.2 (см. §16.123): там заявлено, что можно выписать depth‑$d$ формулы размера $M$,
  ...
  Следующий шаг: проверить, хватает ли заявленной HR‑representability (PARITY на одном блоке $n$ переменных при $d\\approx\\log n/\\log\\log n$, $M=\\mathrm{poly}(n)$) с «съеданием» константной глубины, чтобы отрицательные литералы в §16.130–§16.137 реализовывались как $\\neg P$ (или $\\neg P\\vee\\bot$) без выхода за целевую глубину; если нет — где именно ломается параметрика.
