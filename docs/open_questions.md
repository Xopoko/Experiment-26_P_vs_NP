...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: изолировать «локальный» base‑change для row‑interval’ов — можно ли в bounded‑depth Frege при $d=\\Theta(\\log n/\\log\\log n)$ полиномиально доказывать корректность склейки/сдвига сегмента (формула паритета на интервале длины $t+1$ vs интервал длины $t$ + один литерал)?
