...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  - `Priority:` P1
  - `Status:` ACTIVE
  - `LastStepID:` Q39.S92-globalfixedpair-fixedorientation-contiguous-shift-alt5
  - `NextStepID:` Q39.S93-globalfixedpair-fixedorientation-contiguous-shift-alt6
  ...
  - `Update:` toy‑контрпример при $k=2$: ещё один alt‑shift
    ...
    (Q39_globalfixedpair_fixedorientation_contiguous_shift_alt5_vec1/vec2).
  - `Use:` следующий тест — следующий alt‑shift (alt6) для contiguous blocks.
  ...
  - `PublicSurface:` `formal/WIP/Work.lean` (Q39_rank2_globalfixedpair_fixedorientation_contiguous_shift_alt5)
...
- [ ] **Q43 (flat local‑EF(s): существуют ли “малые” evaluations для poly‑size доказательств?):**
  ...
  - `NextStepID:` Q43.S228-flat-eval-hr-depth-range-constants-a0-c1c2-log2-verify-regime-d-criterion-bound-apply-params-poly
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` no
  - `PublicSurface:` `formal/WIP/Work.lean` (Q43_thm41_regime_d_ok_param_of_le)
  - `Success:` либо схема построения cost‑$t$ evaluations (с $t=\mathrm{polylog}(n)$) для каждой строки flat local‑EF(s)‑доказательства, либо точная точка поломки (где multi‑switching/representation требует nesting или глобальной поддержки)
  - `Lens:` Эквивалентность (уровни ↔ глубина).
  - `Artifact:` Proof.
  - `Update:` режим‑$d$ пакет теперь применён к параметру $N$:
    из $N\\le |F|$ и порога для $|F|$ выводится порог для $N$ при том же bound $|F|\\ge c_1$.
    См. `formal/WIP/Work.lean` (Q43_thm41_regime_d_ok_param_of_le).
  - `Use:` следующий шаг — зафиксировать полиномиальный режим $N=|F|^C$ и связать с $M$.
