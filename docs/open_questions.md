...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: формализовать «base‑change» между разными представлениями паритета, необходимый в стратегии Håstad–Risse, и попытаться доказать/опровергнуть, что bounded‑depth Frege может делать такой перевод при глубине $\\Theta(\\log n/\\log\\log n)$ с polynomial‑size.
