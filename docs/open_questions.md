...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: найти структурный base‑change для нужных разбиений на grid, который избегает blow‑up $k\\to k^2$ (например, цепочка контролируемых refinement/coarsening), или показать, что в стратегии Håstad–Risse неизбежно возникают пары разбиений с $|\\mathcal P\\wedge\\mathcal Q|=\\Omega(k^2)$.
