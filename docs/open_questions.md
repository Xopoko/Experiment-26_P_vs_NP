...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: для HR‑конструкции на одном блоке попытаться (a) построить poly‑size bounded‑depth вывод $N\\leftrightarrow\\neg P$, либо (b) локализовать, почему этот вывод должен быть quasi‑poly (тогда это и есть объяснение «representation, not proof» из §1.2).
