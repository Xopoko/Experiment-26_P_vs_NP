...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  - `Priority:` P1
  - `Status:` ACTIVE
  - `LastStepID:` Q39.S95-globalfixedpair-fixedorientation-contiguous-shift-alt8
  - `NextStepID:` Q39.S96-globalfixedpair-fixedorientation-contiguous-shift-alt9
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` yes
  - `Lens:` Коммуникация/ранг (alternating unit frontier).
  - `Artifact:` Counterexample (fixed orientation).
  - `Update:` toy‑контрпример при $k=2$: следующий циклический alt‑shift
    при global fixed‑pair и fixed orientation всё ещё даёт две ненулевые разные проекции ⇒ ранг 2.
    Toy‑check: попытка «убить» через циклический wrap‑shift не снижает ранг.
    См. `formal/WIP/Work.lean`
    (Q39_globalfixedpair_fixedorientation_contiguous_shift_alt8_vec1/vec2).
  - `Use:` следующий тест — следующий alt‑shift (alt9) для contiguous blocks.
  ...
  - `PublicSurface:` `formal/WIP/Work.lean` (Q39_rank2_globalfixedpair_fixedorientation_contiguous_shift_alt8)
...
- [ ] **Q43 (flat local‑EF(s): существуют ли “малые” evaluations для poly‑size доказательств?):**
  - `Priority:` P0
  - `Status:` ACTIVE
  - `LastStepID:` Q43.S232-flat-eval-hr-depth-range-constants-a0-c1c2-log2-verify-regime-d-criterion-bound-apply-params-poly-compare
  - `NextStepID:` Q43.S233-flat-eval-hr-depth-range-constants-a0-c1c2-log2-verify-regime-d-criterion-bound-apply-params-poly-compare-threshold
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` no
  - `PublicSurface:` `formal/WIP/Work.lean` (Q43_log2_grid_pow_le_mul_succ)
  - `Success:` либо схема построения cost‑$t$ evaluations (с $t=\mathrm{polylog}(n)$) для каждой строки flat local‑EF(s)‑доказательства, либо точная точка поломки (где multi‑switching/representation требует nesting или глобальной поддержки)
  - `Lens:` Эквивалентность (уровни ↔ глубина).
  - `Artifact:` Proof.
  - `Update:` доказано неравенство $\\log_2(|F|^C)\\le (\\log_2|F|+1)C$
    для $|F|=Q43\\_grid\\_size(n)$ (база $\\log_2$, без `exp`).
    Toy‑check: использована монотонность `log2` и оценка $a\\le 2^{\\log_2 a+1}$, контрпримеров нет.
    См. `formal/WIP/Work.lean` (Q43_log2_grid_pow_le_mul_succ).
  - `Use:` следующий шаг — при $n\\ge 2$ заменить $\\log_2|F|+1\\le 2\\log_2|F|$
    и сравнить $2C\\,c_1\\log_2^5|F|$ с $|F|$ для явного порога $|F|\\ge n_0(C)$.
