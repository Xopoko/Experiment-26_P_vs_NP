...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: проверить, можно ли в стратегии Håstad–Risse для grid держать все нужные разбиения 1D‑совместимыми (или локально близкими), иначе зафиксировать место, где неизбежен «поперечный» переход типа row↔column и тогда §16.132 снова даёт quasi‑poly барьер.
