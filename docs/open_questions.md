...
- [ ] **Q39 (сделать верхнюю константу для Tseitin(Grid) явной, без «существует $c$»):**
  В §16.117 сравнение констант сведено к явной нижней 59 (Håstad’20) и неявной верхней $c$ (GIRS’19 Thm. 19).
  Прогресс: в §16.118 зафиксировано $c=21$ для GIRS’19 Lemma 24/25; в §16.119 распакован Claim 28 → Thm. 27
  и получено численное $c=4$ в форме Thm. 19 (при условии $X^{1/d}\\ge\\max(2d,6)$ для $X=\\Delta\\,\\mathrm{tpw}$), т.е.
  верхняя оценка для Tseitin(Grid) стала «в явных числах».
  Следующий шаг: либо убрать/ослабить техническое условие на $X^{1/d}$ (или улучшить $4$ другим выбором $t_i$),
  либо зафиксировать для grid явный верхний коэффициент по глубине и сравнить его с 59 из Håstad’20.

## Завершённые (архив)

- [x] **Q38 (константы в depth‑threshold для Tseitin(Grid): сравнить Håstad’20 и GIRS’19):**
  закрыто: в `docs/research/16_tseitin.md` §16.117 выписано сравнение — нижняя константа 59 явна, верхняя в GIRS’19 задана только через неявную константу $c$,
  поэтому «численное» сравнение требует отдельного распаковки/доп. источника.
- [x] **Q37 (вынести в основной текст краткий итог по Tseitin(Grid): depth = $\\Theta(\\log N/\\log\\log N)$):**
  закрыто: итог добавлен в `docs/15_proof_complexity.md` (рядом с обсуждением Tseitin‑кандидатов); детали и ссылки: §16.92+§16.115–§16.116.
- [x] **Q36 (свести глубину $d$ из GIRS’19 Thm. 19 к глубине в 16.92/16.97 и переписать в терминах $N$):**
  закрыто: согласование «depth = $3d+O(1)$» и перезапись в терминах $N$ сделаны в `docs/research/16_tseitin.md` §16.116.
- [x] **Q35 (квантифицировать верхнюю оценку Thm. 19 (GIRS’19) и сравнить с 16.92):**
  закрыто: в `docs/research/16_tseitin.md` §16.115 извлечена явная форма из доказательства Thm. 19 (GIRS’19, p. 14)
  и посчитано, что для $t=\\Theta(n)$ polynomial‑size достигается уже при $d=\\Theta(\\log n/\\log\\log n)$ (с достаточно большим константным коэффициентом).
