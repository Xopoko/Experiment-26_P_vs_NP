...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  §16.123 фиксирует сам «representation» как явную конструкцию: $\\mathrm{PARITY}$ на $m=(\\log M)^{d-1}$ переменных имеет depth $O(d)$ и размер $\\mathrm{poly}(M)$,
  так что bottleneck Q39 — именно выводимость XOR‑суммы, а не вычислимость паритета формулами.
  Следующий шаг: найти другой upper‑аргумент/источник, который действительно даёт polynomial‑size уже при depth $O(\\log n/\\log\\log n)$, или барьер для такого улучшения в Frege без XOR‑гейтов.
