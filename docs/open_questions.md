...
- [x] **Q8 (Tseitin ⇒ резолюционные нижние оценки):** извлечено из `../resources/downloads/itsykson_oparin_2013_tseitin.pdf` (Cor. 1 + Thm. 1) и зафиксировано как **формально изложенный** шаг `docs/research/16_tseitin_core.md` §16.86: для $\\deg(G)\\le k$ имеем $W\\ge e(G)-1$ и $S\\ge \\exp((e(G)-k-1)^2/|E|)$, значит на bounded‑degree экспандерах резолюция экспоненциальна.
...
- [x] **Q5 (planar 3‑SAT blow‑up для 15.7.4d):** добавлена оценка $|r(\\varphi)|=O(|\\varphi|^2)$ как **формально изложенный** шаг `docs/research/16_encodings.md` §16.83 и 1‑строчная ремарка в Лемме 15.7.4d.
...
- [x] **Q4a (канонизация/ROABP-барьер):** добавлены **формально изложенные** шаги `docs/research/16_encodings.md` §16.81–16.82: Tovey‑splitting даёт (O3, $L\\le 3$)-SAT с линейным blow‑up, а 2‑клаузы устраняются padding‑заменой $(x\\vee\\neg y)\\mapsto(x\\vee x\\vee\\neg y)$, получая 3‑SAT с ограничением «каждая переменная ≤4 вхождения».
...
- [x] **Q1a (Pich–Santhanam 2023):** связана оценка размера счётчика из Леммы 15.7.1d с 3‑CNF‑кодированием через Tseitin: добавлена ремарка рядом с 15.7.1d и **формально изложенный** шаг `docs/research/16_encodings.md` §16.80.
