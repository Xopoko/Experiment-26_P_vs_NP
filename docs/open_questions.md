...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  - `Priority:` P1
  - `Status:` ACTIVE
  - `LastStepID:` Q39.S113-globalfixedpair-fixedorientation-contiguous-shift-alt26
  - `NextStepID:` Q39.S114-globalfixedpair-fixedorientation-contiguous-shift-alt27
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` yes
  - `Lens:` Коммуникация/ранг (alternating unit frontier).
  - `Artifact:` Counterexample (fixed orientation).
  - `Update:` toy‑контрпример при $k=2$: очередной cyclic alt‑shift (alt26)
    при global fixed‑pair и fixed orientation всё ещё даёт две ненулевые разные проекции ⇒ ранг 2.
    См. `formal/WIP/Work.lean`
    (Q39_globalfixedpair_fixedorientation_contiguous_shift_alt26_vec1/vec2).
  - `Use:` следующий тест — следующий alt‑shift (alt27) для contiguous blocks.
  ...
  - `PublicSurface:` `formal/WIP/Work.lean` (Q39_rank2_globalfixedpair_fixedorientation_contiguous_shift_alt26)
