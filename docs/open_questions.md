...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: проверить на стратегии для grid, можно ли вести доказательство так, чтобы каждое локальное обновление границы имело пересечение $O(1)$ и обе строки были записаны как XOR по $O(\\log n)$ блокам одного «текущего» разбиения; иначе выделить минимальный «поперечный» шаг row↔column, где неизбежно включается барьер §16.132.
