...
- [ ] **Q12 (TseitinCNF vs PC при $\mathrm{char}(F)\ne 2$):** для XOR‑системы Tseitin на экспандерах
  известны линейные degree‑LB и exp‑размер PC‑опровержений при $\mathrm{char}(F)\ne 2$ (см. §16.88),
  а для 3‑CNF кодировки при $\mathrm{char}(F)=2$ есть степень 3 (см. §16.89). Вопрос: наследует ли
  3‑CNF кодировка $\mathrm{TseitinCNF}(G,\chi)$ аналогичные degree/size‑нижние оценки при
  $\mathrm{char}(F)\ne 2$? Следующий шаг: проверить/выписать (или найти ссылку на) локальную симуляцию
  XOR‑аксиомы одной вершины через 4 клаузные аксиомы в PC над $\mathrm{char}(F)\ne 2$ с контролем степени.
