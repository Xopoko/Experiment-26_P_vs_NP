...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: вернуться к ядру Q39 и попробовать формализовать/убить toy‑случай синтаксического Gaussian‑шага: из представлений $L_1=b_1$ и $L_2=b_2$ вывести представление $L_1\\oplus L_2=b_1\\oplus b_2$ в bounded‑depth Frege хотя бы при пересечении в одной переменной (аналогия с резолюцией в HR’25 §1.2).
