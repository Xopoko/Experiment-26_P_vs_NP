...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: теперь главный узел — **глубина**: в §16.145–§16.146 мы используем dyadic‑merge для поддержания $k=O(\\log n)$; нужно формально понять, можно ли держать блок‑паритеты глубины $d\\approx\\log n/\\log\\log n$ без накопления $O(\\log n)$ глубины (или выделить место, где неизбежен «дорогой» base‑change, возвращающий барьер §16.132).
