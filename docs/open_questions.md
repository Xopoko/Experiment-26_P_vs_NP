...
- [ ] **Q43 (flat local-EF(s): are there "small" evaluations for poly-size proofs?):**
  ...
  - `NextStepID:` Q43.S284-gap-min-global-le-bridge
  ...
  - `Artifact:` Proof.
  - `Update:` added `Q43_gap_min_ratio_drop_global_grid_of_succ_le` so the strict drop
    can be recovered directly from the +1 bound.
  - `Use:` next: route any remaining gap-min uses through
    `Q43_gap_min_ratio_drop_global_grid_of_succ_le` (avoid re-expanding `Q43_gap_min_ratio_k`).
