...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  режим, где хотелось бы получить polynomial‑size; §16.115+§16.120 дают явный unconditional upper вида $\\mathrm{poly}(|T|)\\cdot 2^{O(d\\cdot X^{2/d})}$ и показывают,
  что из него polynomial‑size следует только при $d=\\Theta(\\log n)$.
  Следующий шаг: найти другой upper‑аргумент/источник, дающий polynomial‑size уже при depth $O(\\log n/\\log\\log n)$ (или барьер для такого улучшения в Frege без XOR‑гейтов).
