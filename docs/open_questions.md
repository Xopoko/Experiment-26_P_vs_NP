...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: свести «локальный» base‑change в bounded‑depth Frege к одной операции merge для дизъюнктных интервалов $J,J'$ и понять, есть ли poly‑size доказательство
  $\\mathrm{PARITY}(J)=b_1,\\ \\mathrm{PARITY}(J')=b_2\\vdash \\mathrm{PARITY}(J\\cup J')=b_1\\oplus b_2$
  при $d=\\Theta(\\log n/\\log\\log n)$.
