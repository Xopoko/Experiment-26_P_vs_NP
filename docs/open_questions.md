...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: совместить §16.144 с §16.143+§16.136 и выписать явный «протокол» поддержания block‑формы для $\\delta(R_{j,t})$ с $k=O(\\log n)$ (локальный XOR‑шаг + $O(\\log n)$ disjoint‑merge), а затем найти, где в стратегии для всего grid неизбежно появляется row↔column base‑change (и включается барьер §16.132), либо доказать, что его можно избежать.
