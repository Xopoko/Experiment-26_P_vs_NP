...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  - `Priority:` P1
  - `Status:` ACTIVE
  - `LastStepID:` Q39.S40-2k-two-strip-chain-lockstep-blocks (см. `formal/Notes/TseitinQ39.lean`)
  - `NextStepID:` Q39.S41-2k-two-strip-chain-lockstep-dyadic-blocks
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` yes
  - `Lens:` Коммуникация/ранг (lock-step updates).
  - `Artifact:` Counterexample (lock-step block-updates).
  - `Update:` toy‑контрпример при $k=2$: цепочка с lock‑step block‑обновлениями
    (оба полосы меняются синхронно на одном и том же блоке) всё равно даёт ранг 2
    (n=4, независимые проекции у $U_1$ и $U_3$). См. `formal/Notes/TseitinQ39.lean` §16.181.
  - `Use:` даже lock‑step обновления по блокам не снижают ранг; нужно усилить геометрию
    (dyadic/ламинарные блоки в фиксированном порядке или иная синхронная структура).
  - `File:` `formal/Notes/TseitinQ39.lean` (§16.181). `InfoGain:` 1.
  - `BarrierCheck:`
  ...
  - `PublicSurface:` `formal/Notes/TseitinQ39.lean` §16.181
