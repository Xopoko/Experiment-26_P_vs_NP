...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  - `Priority:` P1
  - `Status:` ACTIVE
  - `LastStepID:` Q39.S107-globalfixedpair-fixedorientation-contiguous-shift-alt20
  - `NextStepID:` Q39.S108-globalfixedpair-fixedorientation-contiguous-shift-alt21
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` yes
  - `Lens:` Коммуникация/ранг (alternating unit frontier).
  - `Artifact:` Counterexample (fixed orientation).
  - `Update:` toy‑контрпример при $k=2$: очередной циклический alt‑shift
    при global fixed‑pair и fixed orientation всё ещё даёт две ненулевые разные проекции ⇒ ранг 2.
    Toy‑check: wrap‑shift (циклическая тройка) не снижает ранг.
    См. `formal/WIP/Work.lean`
    (Q39_globalfixedpair_fixedorientation_contiguous_shift_alt20_vec1/vec2).
  - `Use:` следующий тест — следующий alt‑shift (alt21) для contiguous blocks.
  ...
  - `PublicSurface:` `formal/WIP/Work.lean` (Q39_rank2_globalfixedpair_fixedorientation_contiguous_shift_alt20)
...
- [ ] **Q43 (flat local‑EF(s): существуют ли “малые” evaluations для poly‑size доказательств?):**
  - `Priority:` P0
  - `Status:` ACTIVE
  - `LastStepID:` Q43.S240-flat-eval-hr-depth-range-constants-a0-c1c2-log2-verify-regime-d-criterion-bound-apply-params-poly-n0-monon-log2-lift
  - `NextStepID:` Q43.S241-flat-eval-hr-depth-range-constants-a0-c1c2-log2-verify-regime-d-criterion-bound-apply-params-poly-n0-threshold-lift
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` no
  - `PublicSurface:` `formal/WIP/Work.lean` (Q43_log2_grid_size_pow5_mono)
  - `Success:` либо схема построения cost‑$t$ evaluations (с $t=\mathrm{polylog}(n)$) для каждой строки flat local‑EF(s)‑доказательства, либо точная точка поломки (где multi‑switching/representation требует nesting или глобальной поддержки)
  - `Lens:` Эквивалентность (уровни ↔ глубина).
  - `Artifact:` Proof.
  - `Update:` добавлена монотонность $\\log_2^5|F|$ по $n$ для $|F(n)|=n^2$,
    через монотонность $\\log_2|F|$ и $a^5$.
    См. `formal/WIP/Work.lean` (Q43_log2_grid_size_pow5_mono).
  - `Use:` следующий шаг — поднять критерий
    $2C\\,c_1\\log_2^5|F|\\le |F|$ от $n_0$ к $n\\ge n_0$
    с использованием монотонности $|F|$ и $\\log_2^5|F|$.
