...
- [ ] **Q39 (Tseitin(Grid): закрыть разрыв по глубине для polynomial‑size):**
  ...
  Следующий шаг: изолировать «локальный» base‑change для row‑interval’ов — может ли bounded‑depth Frege при $d=\\Theta(\\log n/\\log\\log n)$ полиномиально реализовать добавление/удаление одного элемента на границе интервала (без роста глубины и без quasi‑poly blow‑up)?
