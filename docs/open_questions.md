...
- [ ] **Q39 (Tseitin(Grid): depth‑gap для polynomial‑size в bounded‑depth Frege):**
  - `Priority:` P1
  - `Status:` ACTIVE
  - `LastStepID:` Q39.S44-2k-two-strip-chain-dyadic-microsteps-distinct-support (см. `formal/Notes/TseitinQ39.lean`)
  - `NextStepID:` Q39.S45-2k-two-strip-chain-dyadic-microsteps-strip-support
  - `LeanTarget:` formal/WIP/Work.lean
  - `BarrierCheckRequired:` yes
  - `Lens:` Трейд‑офф (distinct support).
  - `Artifact:` Counterexample (distinct-support microsteps).
  - `Update:` toy‑контрпример при $k=2$: distinct-support микрошаги не снижают ранг,
    так как можно добавить вершину вдали от полос и сохранить проекции; независимые
    проекции у $U'_2$ и $U'_4$. См. `formal/Notes/TseitinQ39.lean` §16.185.
  - `Use:` даже distinct-support микрошаги не снижают ранг; нужно требовать distinct support
    **на полосах** (менять сами $S_{j-1}\\cap\\delta(U)$ и $S_{j+1}\\cap\\delta(U)$).
  - `File:` `formal/Notes/TseitinQ39.lean` (§16.185). `InfoGain:` 1.
  - `BarrierCheck:`
  ...
  - `PublicSurface:` `formal/Notes/TseitinQ39.lean` §16.185
