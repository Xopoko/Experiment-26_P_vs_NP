...
## Роли (QUESTION_SETTER / WORKER / SKEPTIC / SUPERVISOR)

- Если ты запущен как **QUESTION_SETTER** (в prompt прямо сказано “QUESTION_SETTER”): твоя задача — **декомпозиция/выбор** следующего микрошагa (3 кандидата → novelty‑check → голосование за 1); **не редактируй файлы и не делай git commit** — только выдай короткий “Question Set”, чтобы следующий WORKER‑прогон мог его выполнить.
...
Если нужен поиск по arXiv, сначала используйте локальный снимок метаданных `resources/downloads/arxiv-metadata-oai-snapshot.json` через `python3 scripts/arxiv_search.py` (например, `--preset pvnp`); web‑поиск включайте только если локального результата недостаточно.
