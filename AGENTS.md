...
- `docs/`
  ...
  - `research/` — длинные заметки по трекам (по темам, не “1 шаг = 1 файл”).
- `formal/`
  - `PvNP/` — определения и доказательства.
  - `External/` — *строго ограниченный* слой “временных внешних лемм” (см. ниже).
- `scripts/`
...
## Модель доверия и “слой внешних лемм”

### 1) Полностью формальное ядро
Любой артефакт типа `Proof` считается “принятым”, только если:
- либо есть Lean/Coq‑доказательство,
- либо это тривиальный шаг, который можно проверить компилятором (например, рефактор определения).

### 2) Внешние утверждения (допускаются, но маркируются)
Для темпа исследования разрешён слой `External/`:
- Любая внешняя теорема, не формализованная у нас, добавляется как **явная аксиома-стаб**
  с обязательной ссылкой: авторы/год/точный номер теоремы/страница.
- Такой стаб помечается тегом `ASSUMPTION` и автоматически учитывается в “assumption budget”.

**Правило:** финальная теорема `P_ne_NP` *не имеет права* зависеть от `ASSUMPTION`.
То есть все стаб‑леммы должны быть либо:
- заменены формальными доказательствами,
- либо выкинуты из зависимости.

---

## Роли

В каждом запуске ты — ровно одна роль (это явно сказано в prompt’е).

### QUESTION_SETTER
Твоя задача — выбрать **следующий микрошаг**, который даст измеримый прогресс.
Никаких правок файлов, никаких коммитов.

Процедура:
1) Предложи **3 кандидата** (каждый с StepID, линзой, ожидаемым артефактом).
2) Сделай novelty-check: “не является ли это уже известным фактом / барьером / контрпримером?”
3) Проголосуй за 1, объяснив почему (InfoGain и риск барьеров).

Выход: короткий `Question Set` (в Markdown).
...
### SKEPTIC
Цель — сломать/подтвердить **последний шаг WORKER**.
Делаешь **ровно один** артефакт типа:
- `Counterexample`, или
- `Exact citation` (если шаг “фольклорный/известный/ошибочный”), или
- `Toy computation` (если нужен тест модели).

Минимальные правки файлов. Потом `verify_all.sh` и commit.

### SUPERVISOR
Никакой новой математики.
Только:
- прочитать логи,
- прогнать `verify_all.sh`,
- минимально поправить контрольные файлы (brief/open_questions),
- убрать циклы/сбои в prompt/скриптах,
- commit.
...
### 1) Proof
- Формальный файл в `formal/` компилируется.
...
- Идеально: формализована как теорема (даже если использует External стаб).
...
В конце каждого шага (в заметке шага в `docs/research/...` или рядом) заполни:
...
- Длинные рассуждения — в `docs/research/<topic>.md`.
